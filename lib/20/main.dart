...
import 'app/views/navigation/app_navigation.dart';
import 'starter/views/splash_page.dart';

void main() {
  runApp(const ManagerWrapper(child: MyApp()));
  GetInstance().put<AppCounterCtrl>(AppCounterCtrl(),);
}
...
class MyApp extends StatelessWidget {
  ...
  void _listenAppStateChange(BuildContext context, AppStatus state) {
    if (state is AppLoadDone) {
      ...
      // context.read<AppCounterBloc>().add(InitEvent(repository));
      Get.find<AppCounterCtrl>().init(repository);
    }
